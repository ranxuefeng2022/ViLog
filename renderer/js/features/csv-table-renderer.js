...
(function() {
  ...
  function bindColumnSelectorEvents() {
    ...
    // 🔧 Vlog电池数据可视化按钮
    const vlogChartBtn = document.getElementById('csvVlogChart');
    if (vlogChartBtn) {
      vlogChartBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        hideContextMenu();
        showVlogChartInNewWindow();
      });
    }
  ...
  function handleWheel(e) {
    ...
    if (e.altKey || (mouseY > containerHeight - 25 && containerWidth < totalWidth)) {
      ...
      const effectiveVisibleHeight = containerHeight - CONFIG.HEADER_HEIGHT - (showHScroll ? scrollSize : 0);
      const maxScroll = Math.max(0, dataHeight - effectiveVisibleHeight);
  ...
  function handleMouseMove(e) {
    ...
    if (isDragging && dragColumnIndex >= 0 && dragOrigWidths) {
      ...
      if (mouseX > leftEdge) {
        ...
        for (let i = 0; i < dragColumnIndex; i++) {
          ...
          if (colWidth <= 0) continue;  // 跳过隐藏的列

          const nextAccumX = accumX + colWidth;
    ...
    // 计算鼠标悬停的列索引（考虑列宽为0的情况）
    let colIdx = -1;
    let colStartX = getColumnStart(0);
    for (let i = 0; i < visibleColumns && i < columnWidths.length; i++) {
      const colWidth = columnWidths[i];
      if (colWidth > 0 && x >= colStartX && x < colStartX + colWidth) {
        colIdx = i;
        break;
      }
      if (colWidth > 0) {
    ...
    for (let i = 0; i < visibleColumns && i < columnWidths.length; i++) {
      // 计算边框位置（在内容坐标系中）
      const borderX = getColumnStart(i) + columnWidths[i];
      // 只有当列宽>0时才显示调整手柄
    ...
    if (showVScroll) {
      ...
      if (x >= vTrackX && y >= headerHeight) {
        if (y >= sliderY && y <= sliderY + sliderHeight) {
          ...
          // 计算有效可视高度（考虑横向滚动条）
          const effectiveVisibleHeight = containerHeight - headerHeight - (showHScroll ? scrollSize : 0);
          scrollY = Math.max(0, Math.min(scrollY, dataHeight - effectiveVisibleHeight));
          requestAnimationFrame(render);
          return;
        }
      }
  ...
  function initColumnWidths() {
    ...
    for (let i = 0; i < csvData.totalColumns; i++) {
      if (i < visibleColumns) {
        ...
        const headerText = csvData.headers[i] || `列${i + 1}`;
        ...
        // 采样更多行以确保覆盖各种数据（至少200行或全部，如果少于200行）
        const sampleRows = Math.min(200, csvData.rows.length);
        for (let j = 0; j < sampleRows; j++) {
          const cellText = String(csvData.rows[j][i] || '');
          const textWidth = ctx.measureText(cellText).width;
          maxContentWidth = Math.max(maxContentWidth, textWidth);
        }

        // 取表头和内容的最大宽度，并添加padding
        const requiredWidth = Math.max(headerWidth, maxContentWidth) + CONFIG.CELL_PADDING_X * 2;

        // 限制在合理范围内：最小80，最大500
        columnWidths[i] = Math.max(80, Math.min(500, requiredWidth));
      } else {
  ...
  function render() {
    ...
    const showHScroll = containerWidth < totalWidth;
    // 计算有效内容高度（减去横向滚动条高度）
    const effectiveContentHeight = showHScroll ? containerHeight - scrollSize : containerHeight;

    const startRow = Math.floor(scrollY / CONFIG.ROW_HEIGHT);
    const endRow = Math.min(csvData.rows.length, Math.ceil((scrollY + effectiveContentHeight) / CONFIG.ROW_HEIGHT) + 1);
  ...
  function showBatteryChart() {
    // 直接使用原始的弹窗图表功能（不检测vlog数据）
    showOriginalChartDialog();
  }

  function showVlogChartInNewWindow() {
    try {
      // 准备数据（转换格式）
      ...
      // 生成完整的HTML
      const htmlContent = generateVlogChartHTML(dataRows);
      ...
      if (newWindow) {
        ...
        showMessage('无法打开新窗口，请检查弹窗拦截设置');
        URL.revokeObjectURL(url);
      }
    } catch (error) {
      console.error('[showVlogChartInNewWindow] Error:', error);
      showMessage('生成图表失败: ' + error.message);
    }
  }

  function generateVlogChartHTML(dataRows) {
    const colors = [
    ...
    // 嵌入数据 - 确保正确转义
    const embeddedData = JSON.stringify(dataRows).replace(/</g, '\\x3C').replace(/>/g, '\\x3E').replace(/&/g, '\\x26');
...
<body>
  <div class="container">
    <header>
      <div>
        <h1>📊 Vlog电池数据可视化</h1>
        <div class="file-info" id="fileInfo">记录数: ${dataRows.length} 条</div>
      </div>
    </header>
    ...
    <div class="chart-card">
      ...
      <div class="controls">
        ...
        <button class="btn btn-secondary" onclick="toggleChargers()">🔌 充电器图标</button>
      </div>
    </div>
  </div>
  ...
  <script>
    // 嵌入的数据
    const rawData = JSON.parse('${embeddedData}');
    ...
    function initChart() {
      ...
      const labels = rawData.map(d => parseTime(d['时间'] || d.time || ''));
      const fieldNames = Object.keys(rawData[0]).filter(k => k !== 'time' && k !== 'version' && k !== '版本');

      const priorityFields = ['电量', '电池电压'];

      const acChargePoints = [];
      const usbChargePoints = [];
      ...
      for (let i = 1; i < chargeStateData.length; i++) {
        if (chargeStateData[i-1] === 0) {
          if (chargeStateData[i] === 1) {
            acChargePoints.push({ x: labels[i], y: 0 });
          } else if (chargeStateData[i] === 2) {
            usbChargePoints.push({ x: labels[i], y: 0 });
          }
        }
      }
            ...
            legend: {
              ...
              onClick: (e, legendItem, legend) => {
                ...
                if (ds.type === 'scatter') {
                  ...
                  // 同步按钮状态
                  chargersVisible = !ds.hidden;
                  // 更新按钮文本
                  const btn = document.querySelector('button[onclick="toggleChargers()"]');
                  if (btn) {
                    btn.textContent = (chargersVisible ? '🔌 隐藏充电器' : '🔌 显示充电器');
                  }
                  ci.update();
                  return;
                }
              ...
              callbacks: {
                ...
                afterBody: function(tooltipItems) {
                  if (tooltipItems.length > 0) {
                    ...
                    return ['', '━━━━━━━━━━━━━━━━', '🌡️ 电池温度: ' + batteryTemp.toFixed(1) + '°C', '🔥 板温: ' + boardTemp.toFixed(1) + '°C'];
                  }
                  return [];
                }
              }
      ...
      chart.data.datasets.forEach((ds, index) => {
        ...
        if (yAxis) {
          if (!allVisible) {
            ...
            yAxis.options.grid.color = 'rgba(255, 255, 255, 0.1)';
            yAxis.options.grid.drawOnChartArea = lineDatasetIndex < 2;
          } else {
    ...
    let chargersVisible = false;
    function toggleChargers() {
      if (!chart) return;
      chargersVisible = !chargersVisible;

      // 切换所有scatter类型的dataset（充电器图标）
      chart.data.datasets.forEach((ds) => {
        if (ds.type === 'scatter') {
          ds.hidden = !chargersVisible;
        }
      });
      chart.update();
      ...
      const btn = document.querySelector('button[onclick="toggleChargers()"]');
      if (btn) {
        btn.textContent = (chargersVisible ? '🔌 显示充电器' : '🔌 隐藏充电器');
      }
    }
  ...
  function showOriginalChartDialog() {
