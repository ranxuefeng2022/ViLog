...
----LOG END----`;
      ...
      const MAX_CACHE_SIZE = 200; // 限制缓存大小，避免内存占用过大（降低到200，减少内存占用）
      ...
      function canLoadMoreContent(newLinesCount = 0) {
        const MAX_MEMORY_MB = 500; // 500MB 上限
        const WARNING_MEMORY_MB = 300; // 300MB 时开始清理缓存

        // 🚀 更精确的内存估算：每行按 300 字节计算（包含字符串、DOM、缓存等开销）
        // JavaScript 字符串对象实际占用比内容本身大得多
        const estimatedMemoryBytes = (originalLines.length + newLinesCount) * 300;
        const estimatedMemoryMB = estimatedMemoryBytes / (1024 * 1024);
        const currentMemoryMB = (originalLines.length * 300) / (1024 * 1024);
      ...
      class DOMPool {
        ...
        release(index) {
          ...
          if (element) {
            // 🚀 性能优化：从 Intersection Observer 中移除
...
✅ 使用文件树的"导入文件"按钮`);
      ...
      function init() {
        ...
        outer.addEventListener("scroll", () => {
          ...
          // 节流：取消之前的未执行请求，只保留最新的
          scrollThrottleId = true;
        }, { passive: true });
...
2. 或直接从压缩软件拖拽文件到本应用`);
      ...
      function processFileContent(content, fileName, filePath = null, fileTreeIndex = null) {
        ...
        if (!canLoadMoreContent(lines.length)) {
          const currentMemoryMB = (originalLines.length * 300) / (1024 * 1024);
          const errorMsg = `⚠️ 内存已满 (${currentMemoryMB.toFixed(1)}MB / 500MB)\n\n无法加载文件：${fileName}\n\n💡 建议：请先清除部分日志（点击文件树中的取消选中），或使用过滤功能。`;

          // 🚀 强制清理：在显示错误前清理所有可能的缓存
          clearHtmlParseCache();

          // 尝试清理 DOM 池
          if (typeof domPool !== 'undefined' && domPool.clear) {
            domPool.clear();
          }
        ...
        // ⚡ 性能优化：禁用从文件名提取基准时间（用户不需要 UTC 时间转换）
        ...
        if (lines.length > 0) {
          ...
          // ⚡ 性能优化：禁用 UTC 参考点提取（用户不需要 UTC 时间转换）
      ...
      async function applyHighlightAsync(showProgress) {
        ...
        try {
          ...
          for (let i = 0; i < lineIndices.length; i++) {
            ...
            const lineElement = inner.querySelector(`[data-index="${lineIndex}"]`);
      ...
      function updateVisibleLines() {
        ...
        if (fragment.children.length > 0) {
          ...
          // 🚀 性能优化：将新元素添加到 Intersection Observer
          if (window.mainLinesObserver) {
            const newElements = fragment.querySelectorAll('.log-line, .file-header');
