...
// 内存字节计数器：避免每次写入都调用 fs.statSync() 阻塞主进程
let logBytesWritten = 0;
// 每 N 次写入才做一次 statSync 校正计数器（防止计数器漂移）
const LOG_STAT_CHECK_INTERVAL = 200;
let logWriteCountSinceStat = 0;
// 日志轮转状态标志，防止轮转过程中数据丢失
...
function writeToLog(level, args) {
  ...
  try {
    ...
    // 用内存计数器判断是否需要轮转（避免每次 fs.statSync 阻塞）
    if (logBytesWritten > LOG_MAX_SIZE) {
      // 定期用 statSync 校正计数器，防止累积误差
...
ipcMain.handle('watch-directory', async (event, dirPath) => {
  try {
    ...
    const watcher = fs.watch(dirPath, { recursive: false }, (eventType, filename) => {
      // Linux inotify 批量事件可能不带 filename，不能丢弃，发全量刷新标记
      if (!filename) {
    ...
    watcher.on('error', (error) => {
      ...
      // ENOSPC 表示 inotify 队列溢出或 watcher 数量达上限，标记以便重建
...
app.on('will-quit', () => {
  ...
  // 关闭关键词数据库，确保 WAL checkpoint
  if (keywordDB) {
    try { keywordDB.close(); } catch(e) { /* 忽略 */ }
    keywordDB = null;
  }
