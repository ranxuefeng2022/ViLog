...
function interpolateUTC(bootTime, referencePoints) {
  ...
  // 二分搜索找到 bootTime 所在的参考点区间，然后线性插值
  let lo = 0, hi = referencePoints.length - 2;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const curr = referencePoints[mid];
    const next = referencePoints[mid + 1];

    if (bootTime < curr.bootTime) {
      hi = mid - 1;
    } else if (bootTime >= next.bootTime) {
      lo = mid + 1;
    } else {
      // 找到区间 [mid, mid+1)
