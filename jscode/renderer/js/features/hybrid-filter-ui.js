    ...
    if (filterBox) {
      ...
      filterBox.addEventListener('keydown', async function(e) {
        if (e.key === 'Enter') {
          ...
          // 🚀 新增：如果过滤面板可见且有结果，则不处理，让导航逻辑生效
          const filteredPanel = document.getElementById('filteredPanel');
          const isFilteredPanelVisible = filteredPanel &&
            filteredPanel.classList.contains('visible');

          const hasFilterResults = window.currentFilter &&
            window.currentFilter.filteredLines &&
            window.currentFilter.filteredLines.length > 0;

          // 如果过滤面板可见且有结果，则不处理，让 original-script.js 的导航逻辑生效
          if (isFilteredPanelVisible && hasFilterResults) {
            console.log('[HybridFilter UI] 过滤面板已可见，跳过 ripgrep 处理，使用导航逻辑');
            return;
          }
