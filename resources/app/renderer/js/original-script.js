...
----LOG END----`;
      ...
      // 🚀 性能优化：HTML解析结果缓存
      // 使用LRU缓存避免重复解析相同的HTML内容
      const htmlParseCache = new Map();
      const MAX_CACHE_SIZE = 500; // 限制缓存大小，避免内存占用过大
      let cacheAccessOrder = []; // 用于追踪缓存访问顺序

      /**
       * 🚀 从缓存获取解析后的HTML
       * @param {string} text - 原始文本
       * @param {string} cacheKey - 缓存键（可以包含关键词信息）
       * @returns {string|null} 缓存的HTML，如果不存在则返回null
       */
      function getCachedHtml(text, cacheKey) {
        const key = `${cacheKey}:${text.slice(0, 100)}`; // 使用前100个字符作为键的一部分
      ...
      const bookmarkModalMeta = document.getElementById("bookmarkModalMeta");
      const bookmarkModalText = document.getElementById("bookmarkModalText");
      const bookmarkModalNote = document.getElementById("bookmarkModalNote");
      ...
      // 性能优化：过滤缓存
      let unescapeCache = new Map(); // 反转义缓存
      let lastFilterCacheKey = "";   // 上次过滤的缓存键
      const MAX_UNESCAPE_CACHE_SIZE = 10000; // 缓存大小限制
      ...
      // UTC 时间转换基准时间（从文件名中提取）
      let utcBaseTimestamp = null;

      // UTC参考点列表（从日志内容中提取）- 使用全局变量
      window.utcReferencePoints = [];
       ...
       * 使用WASM批量转换UTC时间
       */
      async function convertUTCTimeWithWasm() {
        if (!wasmConverter) {
          wasmConverter = new WasmTimestampConverter();
        }

        try {
          showMessage("正在初始化WASM转换器...");

          // 初始化WASM
          const initialized = await wasmConverter.init();
          if (!initialized) {
            throw new Error('WASM初始化失败');
          }

          showMessage("正在提取UTC参考点...");

          // 检查UTC参考点
          if (!window.utcReferencePoints || window.utcReferencePoints.length === 0) {
            await extractUTCReferencePointsAsync();
          }

          if (!window.utcReferencePoints || window.utcReferencePoints.length === 0) {
            showMessage('⚠️ 无法提取UTC参考点，无法进行时间转换\n请确保日志中包含 "android time" 标记行');
            return;
          }

          showMessage(`正在设置${window.utcReferencePoints.length}个参考点...`);

          // 转换参考点格式为WASM需要的格式
          const wasmRefs = window.utcReferencePoints.map(ref => ({
            bootTime: ref.bootTime,
            utcTime: new Date(ref.utcTime).getTime()
          }));

          // 设置参考点
          await wasmConverter.setReferencePoints(wasmRefs);

          showMessage("正在使用WASM转换时间戳...");

          // 执行批量转换
          const startTime = performance.now();

          const results = await wasmConverter.convertBatch(originalLines, {
            batchSize: 10000,
            onProgress: (progress) => {
              console.log(`[WASM] 转换进度: ${progress.progress}%`);
            }
          });
          ...
          if (results && results.results) {
            ...
            originalLines.push(...results.results);

            console.log(`✅ WASM转换完成: ${results.convertedCount} 行转换成功, ${results.skippedCount} 行跳过, 耗时 ${elapsed.toFixed(2)}ms`);
            showMessage(`✓ 已使用WASM转换 ${results.convertedCount} 行时间戳 (耗时 ${elapsed.toFixed(0)}ms)`);
          ...
          console.error('[WASM] UTC转换错误:', error);
          showMessage(`WASM转换出错: ${error.message}`);
        }
      }

      /**
       * 异步提取UTC参考点（使用Promise包装）
       */
      function extractUTCReferencePointsAsync() {
        return new Promise((resolve, reject) => {
          try {
            extractUTCReferencePoints(originalLines);
            resolve();
          } catch (error) {
            reject(error);
          }
      ...
      // ============ 书签/注释 ============
      const BOOKMARKS_STORAGE_KEY = "logtool_bookmarks_v1";
      let bookmarks = []; // {id, originalIndex, filePath, lineText, note, createdAt}
      let bookmarkedIndexSet = new Set(); // originalIndex set（当前会话）
      ...
      class DOMPool {
        ...
        _createPoolElement() {
          ...
          element.style.cssText = "position:absolute;width:100%;display:none;will-change:transform;";
          return element;
        }
        ...
        acquire(index, className) {
          ...
          element.style.width = "100%"; // 确保宽度正确
        ...
        release(index) {
          ...
          if (element) {
            // 清理元素状态 - 重置所有可能影响布局的样式
            element.style.display = "none";
            element.style.left = "";
            element.style.top = "";
            element.style.transform = "";
            element.style.width = "";
            // 🚀 性能优化：使用textContent而不是innerHTML来清空内容
            element.textContent = "";
            element.className = "";
      ...
      function getCurrentBookmarksSessionKey() {
        // 尽量稳定：同一批文件/同一路径下复用书签
        try {
          if (isServerMode) {
            const addr = (serverBaseUrl || "").replace(/^https?:\/\//, "");
            const sel = Array.isArray(selectedFiles) ? selectedFiles.length : 0;
            return `server:${addr}|path:${serverCurrentPath || ""}|sel:${sel}`;
          }
          // 本地：用文件名集合+总行数做一个轻量 key（避免存储过大）
          const names = (Array.isArray(currentFiles) ? currentFiles : [])
            .slice(0, 50)
            .map((f) => (f?.webkitRelativePath || f?.name || "").toString())
            .join("|");
          return `local:names:${names}|lines:${originalLines.length}`;
        } catch (_) {
          return "default";
        }
      }

      function loadBookmarksForCurrentSession() {
        const key = getCurrentBookmarksSessionKey();
        let stored = null;
        try {
          stored = JSON.parse(localStorage.getItem(BOOKMARKS_STORAGE_KEY) || "null");
        } catch (_) {
          stored = null;
        }
        const list = stored?.bySession?.[key];
        bookmarks = Array.isArray(list) ? list : [];
        // 清理非法项
        bookmarks = bookmarks
          .filter((b) => b && typeof b.originalIndex === "number" && typeof b.id === "string")
          .map((b) => ({
            id: String(b.id),
            originalIndex: b.originalIndex | 0,
            filePath: b.filePath != null ? String(b.filePath) : "",
            lineText: b.lineText != null ? String(b.lineText) : "",
            note: b.note != null ? String(b.note) : "",
            createdAt: b.createdAt != null ? String(b.createdAt) : "",
          }));
        bookmarkedIndexSet = new Set(bookmarks.map((b) => b.originalIndex));
      ...
      function saveBookmarksForCurrentSession() {
        const key = getCurrentBookmarksSessionKey();
        let stored = { version: 1, bySession: {} };
        try {
          stored = JSON.parse(localStorage.getItem(BOOKMARKS_STORAGE_KEY) || "null") || stored;
        } catch (_) {}
        if (!stored.bySession || typeof stored.bySession !== "object") stored.bySession = {};
        stored.bySession[key] = bookmarks;
        try {
          localStorage.setItem(BOOKMARKS_STORAGE_KEY, JSON.stringify(stored));
        } catch (_) {
          // ignore
        }
        ...
        if (bookmarksBtn) {
          if (bookmarks.length > 0) {
            ...
            bookmarksBtn.classList.remove('has-bookmarks');
          }
        }
      }
      ...
      function getBookmarkByOriginalIndex(originalIndex) {
        return bookmarks.find((b) => b.originalIndex === originalIndex) || null;
      }
      ...
      function bookmarkSelectedLine() {
        // 🚀 优先使用过滤面板选中行，其次使用主日志框选中行
        let idx = -1;

        // 检查过滤面板是否有选中行
        if (lastClickedFilteredIndex >= 0 && filteredPanelAllOriginalIndices && filteredPanelAllOriginalIndices[lastClickedFilteredIndex] !== undefined) {
          idx = filteredPanelAllOriginalIndices[lastClickedFilteredIndex];
        } else if (selectedOriginalIndex >= 0) {
          idx = selectedOriginalIndex;
        } else if (currentPermanentHighlightIndex >= 0) {
          idx = currentPermanentHighlightIndex;
        }

        if (idx < 0) {
          showMessage("请先点击一行日志，再按 Ctrl+B 添加书签");
          return;
        }
        toggleBookmarkForOriginalIndex(idx, true);
      }

      // 🚀 Ctrl+B：自动添加当前行到书签（不弹框，自动编号）
      function bookmarkSelectedLineNoPrompt() {
        // 🚀 优先使用过滤面板选中行，其次使用主日志框选中行
        let idx = -1;
        ...
        // 不弹框，不打开编辑器
        toggleBookmarkForOriginalIndex(idx, false);
      }

      function toggleBookmarkForOriginalIndex(originalIndex, openEditorIfAdded) {
        if (originalIndex < 0 || originalIndex >= originalLines.length) return;
        const existing = getBookmarkByOriginalIndex(originalIndex);
        if (existing) {
          bookmarks = bookmarks.filter((b) => b.id !== existing.id);
          bookmarkedIndexSet.delete(originalIndex);
          saveBookmarksForCurrentSession();
          renderBookmarksPanelList();
        ...
        const filePath = getFileNameForLineIndex(originalIndex) || "";
        const raw = originalLines[originalIndex] || "";
        const lineText = raw; // 直接使用原始内容

        // 🚀 自动生成书签编号（从1开始）
        const bookmarkNumber = bookmarks.length + 1;
        ...
        const bm = {
          ...
          filePath,
          lineText,
          note: `#${bookmarkNumber}`, // 自动编号作为注释
          createdAt: new Date().toISOString(),
          bookmarkNumber, // 保存编号用于显示
        };
        bookmarks.unshift(bm);
        bookmarkedIndexSet.add(originalIndex);
        saveBookmarksForCurrentSession();
        renderBookmarksPanelList();
        try { forceUpdateVisibleLines(); } catch (_) {}

        // 只在手动添加时显示消息
        if (openEditorIfAdded) {
          showMessage(`已添加书签 #${bookmarkNumber}`);
        } else {
          // Ctrl+B 自动添加时不显示消息
        }

        if (openEditorIfAdded) openBookmarkModal(bm.id);
      }
      ...
      function toggleBookmarksPanel(force) {
        ...
        if (typeof force === "boolean") bookmarksPanelVisible = force;
        else bookmarksPanelVisible = !bookmarksPanelVisible;
        bookmarksPanel.classList.toggle("visible", bookmarksPanelVisible);
        bookmarksPanel.setAttribute("aria-hidden", bookmarksPanelVisible ? "false" : "true");
        renderBookmarksPanelList();
        if (bookmarksPanelVisible) {
          try { bookmarksSearch?.focus(); } catch (_) {}
        }
      }
      ...
      function renderBookmarksPanelList() {
        ...
        const q = (bookmarksSearch?.value || "").toLowerCase();
        const list = !q
          ? bookmarks
          : bookmarks.filter((b) => {
              const hay = `${b.filePath}\n${b.lineText}\n${b.note}`.toLowerCase();
              return hay.includes(q);
            });
        if (list.length === 0) {
          bookmarksList.innerHTML = `<div class="bm-empty">${bookmarks.length === 0 ? "暂无书签。点击一行日志后按 Ctrl+B 可添加。" : "没有匹配的书签。"}</div>`;
          return;
        }
        ...
        for (const b of list) {
          ...
          // 🚀 计算书签编号（因为是unshift添加的，需要反向计算）
          const displayNumber = b.bookmarkNumber || (bookmarks.indexOf(b) + 1);

          const fileShort = (b.filePath || "").split(/[/\\]/).pop() || b.filePath || "(未知文件)";
          // 🚀 新的书签项布局：编号 | 内容行 | 删除按钮
          card.innerHTML =
            `<div class="bm-main">` +
            `<div class="bm-number">#${displayNumber}</div>` +
            `<div class="bm-content">` +
            `<div class="bm-file" title="${b.filePath || ""}">${fileShort}</div>` +
            `<div class="bm-text" title="${b.lineText || ""}">${b.lineText || ""}</div>` +
            `</div>` +
            `<button class="bm-delete" data-id="${b.id}" title="删除书签">×</button>` +
            `</div>`;
        ...
        // 🚀 绑定删除按钮事件
        const deleteButtons = bookmarksList.querySelectorAll('.bm-delete');
          ...
          btn.addEventListener('click', (e) => {
            ...
            if (id) {
              ...
              if (bm) {
                ...
                bookmarkedIndexSet.delete(bm.originalIndex);
                saveBookmarksForCurrentSession();
                renderBookmarksPanelList();
                try { forceUpdateVisibleLines(); } catch (_) {}
              }
      ...
      function openBookmarkModal(bookmarkId) {
        const bm = getBookmarkById(bookmarkId);
        if (!bm || !bookmarkModal) return;
        bookmarkModalEditingId = bm.id;
        const fileShort = (bm.filePath || "").split(/[/\\]/).pop() || bm.filePath || "(未知文件)";
        if (bookmarkModalMeta) {
          bookmarkModalMeta.textContent = `${fileShort}  ·  第 ${bm.originalIndex + 1} 行`;
          bookmarkModalMeta.title = bm.filePath || "";
        }
        if (bookmarkModalText) bookmarkModalText.textContent = bm.lineText || "";
        if (bookmarkModalNote) bookmarkModalNote.value = bm.note || "";
        bookmarkModal.classList.add("visible");
        bookmarkModal.setAttribute("aria-hidden", "false");
        try { bookmarkModalNote?.focus(); } catch (_) {}
      }
      ...
      function saveBookmarkNoteFromModal() {
        const id = bookmarkModalEditingId;
        if (!id) return closeBookmarkModal();
        const bm = getBookmarkById(id);
        if (!bm) return closeBookmarkModal();
        bm.note = (bookmarkModalNote?.value || "").toString();
        saveBookmarksForCurrentSession();
        renderBookmarksPanelList();
        closeBookmarkModal();
      }

      function deleteBookmarkFromModal() {
        const id = bookmarkModalEditingId;
        if (!id) return closeBookmarkModal();
        const bm = getBookmarkById(id);
        if (!bm) return closeBookmarkModal();
      ...
      async function exportBookmarksAsJson() {
        const payload = {
          version: 1,
          sessionKey: getCurrentBookmarksSessionKey(),
          exportedAt: new Date().toISOString(),
          bookmarks,
        };
        const text = JSON.stringify(payload, null, 2);
        await copyTextToClipboard(text);
        showMessage("书签JSON已复制到剪贴板");
      }
      ...
      async function exportBookmarksAsMarkdown() {
        ...
        lines.push(`- 会话: ${getCurrentBookmarksSessionKey()}`);
        lines.push("");
        for (const b of bookmarks) {
          const fileShort = (b.filePath || "").split(/[/\\]/).pop() || b.filePath || "(未知文件)";
          lines.push(`- **${fileShort}** · L${b.originalIndex + 1}`);
          if (b.note) lines.push(`  - 注释: ${b.note.replace(/\n/g, "  \n    ")}`);
          if (b.lineText) lines.push(`  - 内容: \`${b.lineText.slice(0, 200)}\``);
        }
        await copyTextToClipboard(lines.join("\n"));
        showMessage("书签Markdown已复制到剪贴板");
      }

      function importBookmarksFromJson() {
        const raw = prompt("粘贴书签JSON（将合并到当前会话）");
        if (!raw) return;
        let obj = null;
        try {
          obj = JSON.parse(raw);
        ...
        const incoming = Array.isArray(obj?.bookmarks) ? obj.bookmarks : null;
        if (!incoming) {
          showMessage("JSON格式不正确：缺少 bookmarks 数组");
          return;
        }
        const byIndex = new Map(bookmarks.map((b) => [b.originalIndex, b]));
        let merged = 0;
        for (const b of incoming) {
          const idx = Number(b?.originalIndex);
          if (!Number.isFinite(idx) || idx < 0 || idx >= originalLines.length) continue;
          const cur = byIndex.get(idx);
          if (cur) {
            // 合并注释（优先导入的）
            if (b.note) cur.note = String(b.note);
          } else {
            const filePath = getFileNameForLineIndex(idx) || String(b.filePath || "");
            const lineText = originalLines[idx] || ""; // 直接使用原始内容
            bookmarks.unshift({
              id: makeBookmarkId(),
              originalIndex: idx | 0,
              filePath,
        ...
        renderBookmarksPanelList();
        try { forceUpdateVisibleLines(); } catch (_) {}
        showMessage(`已导入/合并 ${merged} 条书签`);
      }

      function initBookmarks() {
        // 初始加载（基于 demo/sessionKey）
        loadBookmarksForCurrentSession();

        if (bookmarksSearch) {
          bookmarksSearch.addEventListener("input", () => renderBookmarksPanelList());
        }
        ...
        if (bookmarksList) {
          bookmarksList.addEventListener("click", (e) => {
            ...
            if (!bm) return;
            // 单击：跳转到原始日志行
            jumpToOriginalLine(bm.originalIndex);
            // 🚀 同时检查过滤面板是否包含该行，如果有也跳转
            jumpToFilteredPanelIfContains(bm.originalIndex);
          });
        ...
        document.addEventListener("keydown", (e) => {
          ...
          if (e.key === "Escape") {
            // Esc 先关 modal，再关 panel
            if (bookmarkModal && bookmarkModal.classList.contains("visible")) {
              e.preventDefault();
              closeBookmarkModal();
              return;
            }
      ...
      let filteredPanelVisibleStart = 0;
      let filteredPanelVisibleEnd = 0;
...
✅ 使用文件树的"导入文件"按钮`);
      ...
      function safeHighlight(text, keyword, replacement) {
        ...
        if (text.indexOf('<') === -1) {
          try {
            ...
            // 降级：大小写不敏感的字符串替换
            return simpleCaseInsensitiveReplace(text, keyword, replacement);
          }
        }
      ...
      async function applySecondaryFilter() {
        ...
        // 🚀 记录当前选中行的原始索引，用于过滤后自动跳转
        let rememberedOriginalIndex = -1;
        if (lastClickedFilteredIndex >= 0 && filteredPanelAllOriginalIndices) {
      ...
      function renderWithDOM(lines, primaryIndices, originalIndices, keywords) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          // 检查是否是书签
          const oi = Number(originalIndex);
          if (Number.isFinite(oi) && bookmarkedIndexSet && bookmarkedIndexSet.has(oi)) {
      ...
      function createFilteredLineElement(index) {
        ...
        // 书签标记（过滤结果中也可见）
        const oi = Number(filteredPanelAllOriginalIndices[index]);
        if (Number.isFinite(oi) && bookmarkedIndexSet && bookmarkedIndexSet.has(oi)) {
          lineElement.classList.add("bookmarked");
        }
      ...
      const DOMCache = {
        ...
        init() {
          ...
          // 主日志框相关
          this._cache.inner = document.getElementById('inner');
          this._cache.outer = document.getElementById('outer');
      ...
      function init() {
        ...
        if (window.electronAPI && window.electronAPI.on) {
          window.electronAPI.on('uart-log-data', (data) => {
            ...
            if (newLineCount > 0) {
              ...
              if (placeholder) {
                placeholder.style.height = (originalLines.length * lineHeight) + 'px';
              }
        ...
        renderLogLines();
        initBookmarks(); // 初始化书签/注释面板（依赖 originalLines）
      ...
      function initFilteredPanel() {
        ...
        */
        ...
        const filteredPanelMaximize = document.getElementById('filteredPanelMaximize');
        const filteredPanelClose = document.getElementById('filteredPanelClose');
      ...
      function initFilteredPanelSearch() {
        ...
        // 搜索框事件：按Enter搜索，Shift+Enter向上搜索
        ...
        filteredPanelSearchBox.addEventListener("keydown", (e) => {
          ...
          if (e.key === "Enter") {
            ...
            if (currentValue !== filteredPanelSearchKeyword) {
              // 关键词变化：重新搜索
      ...
      function filteredPanelJumpToMatch(index) {
        ...
        if (!lineHeight || lineHeight <= 0 || isNaN(lineHeight)) {
          try {
            const virtualContent = document.getElementById('filteredPanelVirtualContent');
      ...
      function highlightFilteredPanelSearchMatch(filteredIndex) {
        ...
        // 确保目标行在可见区域内（如果不在，updateFilteredPanelVisibleLines应该已经更新了）
        // 再次更新可见行以确保目标行被渲染
        updateFilteredPanelVisibleLines();
      ...
      function handleFilteredPanelScroll() {
        ...
        filteredPanelScrollRafId = requestAnimationFrame(() => {
          ...
          // 🔧 修复：滚动停止后延迟 100ms 再次更新，强制重新计算高亮
          // 这样可以确保缓冲区的行在滚动停止后被正确高亮
          filteredPanelScrollDebounce = setTimeout(() => {
            // 🚀 性能优化：移除延迟更新时的头部高度检查
            updateFilteredPanelVisibleLines(true);  // 传入 true 强制重新计算高亮
            filteredPanelScrollDebounce = null;
          }, 100);
        });
      }
      ...
      function getFilteredLineCacheKey(lineIndex, isFileHeader, originalIndex) {
        // 缓存键包含：行索引、是否文件头、原始索引、关键词、搜索词、自定义高亮、缓存版本
        const filterKey = (currentFilter.filterKeywords || []).join('|');
        ...
        return `v${filteredLineCacheVersion}|${lineIndex}|${isFileHeader}|${originalIndex}|${filterKey}|${searchKey}|${secondaryKey}|${customKey}`;
      }
      ...
      function updateFilteredPanelVisibleLines(forceHighlight = false) {
        ...
        // 🚀 性能优化：基于屏幕可见行数动态调整缓冲区大小
        const screenVisibleLines = Math.ceil(filteredPanelContent.clientHeight / filteredPanelLineHeight);
        const totalLines = filteredPanelAllLines.length;

        // 根据文件大小调整缓冲区倍数
        if (totalLines < 100) {
          filteredPanelBuffer = Math.max(20, Math.floor(screenVisibleLines * 2));  // 小文件：2倍缓冲
        } else if (totalLines < 10000) {
          filteredPanelBuffer = Math.max(30, Math.floor(screenVisibleLines * 1.5));  // 中等文件：1.5倍缓冲
        } else if (totalLines < 100000) {
          filteredPanelBuffer = Math.max(40, Math.floor(screenVisibleLines * 1.2));  // 大文件：1.2倍缓冲
        } else {
          filteredPanelBuffer = Math.max(50, Math.floor(screenVisibleLines * 1.0));  // 超大文件：1倍缓冲
        }
        ...
        // 🚀 懒加载优化：计算真正可见区域（不含缓冲区），只对可见区域计算高亮
        const trulyVisibleStart = Math.floor(scrollTop / filteredPanelLineHeight);
        ...
        const hasSearchKeyword = filteredPanelSearchKeyword && filteredPanelTotalMatchCount > 0;
        const hasCustomHighlights = customHighlights && customHighlights.length > 0;

        // 🚀 全局高亮需求：任何行是否需要高亮
        const anyHighlightNeeded = hasPrimaryKeywords || hasSecondaryKeywords || hasSearchKeyword || hasCustomHighlights;
        ...
        if (!anyHighlightNeeded) {
          ...
          for (let i = newVisibleStart; i <= newVisibleEnd; i++) {
            ...
            lineElement.style.cssText = `top:${Math.floor(i * filteredPanelLineHeight)}px;width:100%;position:absolute;`;
            lineElement.title = lineContent;
        ...
        for (let i = newVisibleStart; i <= newVisibleEnd; i++) {
          ...
          if (displayText === undefined) {
            ...
            if (needsHighlight) {
              ...
              // 应用多关键词过滤高亮（如果二级过滤激活则跳过）
              if (!skipPrimaryHighlight && hasPrimaryKeywords) {
                for (let k = 0; k < currentFilter.filterKeywords.length; k++) {
                  const keyword = currentFilter.filterKeywords[k];
                  if (!keyword) continue;
              ...
              if (hasSearchKeyword) {
                // 🚀 注意：由于文本已转义，关键词也需要转义才能匹配
                const escapedKeyword = escapeHtml(filteredPanelSearchKeyword);
                displayText = safeHighlight(
                  displayText,
                  escapedKeyword,
                  (match) => {
                    const isCurrent =
                      filteredPanelTotalMatchCount > 0 &&
                      i === filteredPanelSearchMatches[filteredPanelCurrentMatchIndex] &&
                      match.toLowerCase() === escapedKeyword.toLowerCase();
                    return isCurrent
                      ? `<span class="current-search-highlight">${match}</span>`
          ...
          htmlArray[arrayIndex++] = `<div class="${className}${highlighted}" data-original-index="${originalIndex}" data-filtered-index="${i}" style="top:${top}px;width:100%;position:absolute;">${displayText}</div>`;
        }
      ...
      function handleFilteredLineClick(e) {
        ...
        console.log(`[Click] 记录点击: filteredIndex=${filteredIndex}, originalIndex=${originalIndex}`);
      ...
      function initHeaderHeightProtector() {
        ...
        const header = document.getElementById('filteredPanelHeader');
        if (!header) {
        ...
        const CHECK_INTERVAL = 1000; // 🚀 每秒最多检查一次
        ...
        const enforceHeaderHeight = () => {
          ...
          try {
            ...
            // 🔧 修复：检查完整的 padding（上右下左：0 8px）
            if (currentPaddingTop !== '0px' || currentPaddingRight !== '8px' ||
                currentPaddingBottom !== '0px' || currentPaddingLeft !== '8px') {
      ...
      function initLogContentContextMenu() {
        ...
        // 时间戳转换函数：将纳秒时间戳转换为本地时间格式（参考klog脚本的逻辑）
        function convertTimestampToUTC(lineContent) {
          // 匹配日志行中的时间戳格式: 3,394851,2012655346,-,caller=...
          // 第三个数字是纳秒时间戳（相对于系统启动时间）
          const match = lineContent.match(/^(\d+),(\d+),(\d+),(.*)/);
          if (!match) {
            console.debug(`跳过行（格式不匹配）: ${lineContent.substring(0, 50)}...`);
            return null; // 不是有效的日志行格式
          }

          const [, first, second, nanosStr, rest] = match;
          const nanos = parseInt(nanosStr, 10);
          const bootTimeMicros = Math.floor(nanos / 1000); // 转换为微秒
          const remainingMicros = nanos % 1000000;

          // 调试：输出第一行的计算过程
          if (first === '540718') {
            console.log(`=== 调试第2行（540718）的UTC转换 ===`);
            console.log(`  纳秒时间戳: ${nanos}`);
            console.log(`  bootTimeMicros: ${bootTimeMicros} 微秒`);
            console.log(`  remainingMicros: ${remainingMicros}`);
          }

          // 【关键改进】使用线性插值确保时间单调递增
          // 使用全局变量 window.utcReferencePoints
          if (window.utcReferencePoints && window.utcReferencePoints.length > 0) {
            let localTime = null;
            ...
            for (let i = 0; i < window.utcReferencePoints.length; i++) {
              const ref = window.utcReferencePoints[i];
              if (bootTimeMicros >= ref.bootTime) {
                prevRef = ref;
              } else {
                nextRef = ref;
                break;
            ...
            if (first === '540718') {
              console.log(`  插值区间:`);
              console.log(`    prevRef: ${prevRef ? `boot=${prevRef.bootTime}, time=${prevRef.utcTime.toISOString().substring(0, 19)}` : 'null'}`);
              console.log(`    nextRef: ${nextRef ? `boot=${nextRef.bootTime}, time=${nextRef.utcTime.toISOString().substring(0, 19)}` : 'null'}`);
            }

            if (prevRef && nextRef) {
              // 在两个参考点之间，使用线性插值
              const ratio = (bootTimeMicros - prevRef.bootTime) / (nextRef.bootTime - prevRef.bootTime);
              const prevTime = prevRef.utcTime.getTime();
              const nextTime = nextRef.utcTime.getTime();
              const interpolatedTime = prevTime + ratio * (nextTime - prevTime);
              localTime = new Date(interpolatedTime);

              if (first === '540718') {
                console.log(`  使用线性插值:`);
                console.log(`    ratio: ${ratio.toFixed(6)}`);
                console.log(`    插值结果: ${localTime.toISOString()}`);
              }
            } else if (prevRef) {
              // 只有前一个参考点（boot_time大于所有参考点）
              const bootDuration = bootTimeMicros - prevRef.bootTime;
              localTime = new Date(prevRef.utcTime.getTime() + bootDuration / 1000);

              if (first === '540718') {
                console.log(`  使用前向延伸 (prevRef):`);
                console.log(`    bootDuration: ${bootDuration} 微秒 = ${bootDuration / 1000} 毫秒`);
                console.log(`    计算结果: ${localTime.toISOString()}`);
              }
            } else if (nextRef) {
              // 只有后一个参考点（boot_time小于所有参考点）
              const bootDuration = nextRef.bootTime - bootTimeMicros;
              localTime = new Date(nextRef.utcTime.getTime() - bootDuration / 1000);

              if (first === '540718') {
                console.log(`  使用后向延伸 (nextRef):`);
                console.log(`    bootDuration: ${bootDuration} 微秒 = ${bootDuration / 1000} 毫秒`);
                console.log(`    计算结果: ${localTime.toISOString()}`);
              }
            }
            ...
            if (first === '540718') {
              console.log(`  计算后的本地时间: ${localTime.toISOString()}`);
              console.log(`  本地时间（字符串）: ${localTime.toString()}`);
            }

            // 格式化为 [YYYY-MM-DD HH:MM:SS.mmmmmm]（使用本地时间，不是UTC）
            const year = localTime.getFullYear();
            const month = String(localTime.getMonth() + 1).padStart(2, '0');
            const day = String(localTime.getDate()).padStart(2, '0');

            // 🔧 修复：正确处理微秒部分
            // remainingMicros 是纳秒时间戳的余数（0-999999 微秒）
            // localTime 已经基于 bootTimeMicros / 1000 计算，包含了大部分毫秒
            // 我们只需要从 remainingMicros 中提取毫秒和微秒部分来修正显示

            // remainingMicros 分为：毫秒部分(0-999) 和 微秒部分(0-999)
            const microsMillis = Math.floor(remainingMicros / 1000);  // 余数中的毫秒
            const microsOnly = remainingMicros % 1000;              // 余数中的微秒

            // localTime 的毫秒 + remainingMicros 的毫秒部分
            let totalMillis = localTime.getMilliseconds() + microsMillis;
            let finalSeconds = localTime.getSeconds();
            let finalMinutes = localTime.getMinutes();
            let finalHours = localTime.getHours();

            // 处理毫秒溢出到秒
            if (totalMillis >= 1000) {
              const overflowSeconds = Math.floor(totalMillis / 1000);
              totalMillis = totalMillis % 1000;
              finalSeconds += overflowSeconds;

              if (finalSeconds >= 60) {
                const overflowMinutes = Math.floor(finalSeconds / 60);
                finalSeconds = finalSeconds % 60;
                finalMinutes += overflowMinutes;

                if (finalMinutes >= 60) {
                  const overflowHours = Math.floor(finalMinutes / 60);
                  finalMinutes = finalMinutes % 60;
                  finalHours += overflowHours;
                }
              }
            }

            const finalMilliseconds = String(totalMillis).padStart(3, '0');
            const finalMicroseconds = String(microsOnly).padStart(3, '0');

            // 使用修复后的毫秒和微秒（包含进位处理）
            const localTimestamp = `[${year}-${month}-${day} ${String(finalHours).padStart(2, '0')}:${String(finalMinutes).padStart(2, '0')}:${String(finalSeconds).padStart(2, '0')}.${finalMilliseconds}${finalMicroseconds}]`;

            // 🔧 单调性检查：在所有时间计算完成后进行
            // 将时间戳解析回毫秒数进行比较
            const timestampMatch = localTimestamp.match(/\[(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})\.(\d{6})\]/);
            if (timestampMatch) {
              const [, tYear, tMonth, tDay, tHours, tMinutes, tSeconds, tMicros] = timestampMatch;
              const currentDate = new Date(
                parseInt(tYear), parseInt(tMonth) - 1, parseInt(tDay),
                parseInt(tHours), parseInt(tMinutes), parseInt(tSeconds),
                parseInt(tMicros.substring(0, 3))  // 毫秒部分
              );
              const currentMicros = parseInt(tMicros);

              // 🔧 调试：输出特定行的转换信息
              if (first === '211689' || first === '211690') {
                console.log(`=== 单调性检查 (first=${first}, boot_time=${bootTimeMicros}) ===`);
                console.log(`  转换后时间戳: ${localTimestamp}`);
                console.log(`  currentDate: ${currentDate.toISOString()}`);
                console.log(`  currentTime (ms): ${currentDate.getTime()}`);
                console.log(`  currentMicros: ${currentMicros}`);
                console.log(`  currentTimeInMicros: ${currentDate.getTime() * 1000 + currentMicros}`);
                if (window.lastConvertedTime && window.lastConvertedDate) {
                  const lastTime = window.lastConvertedDate.getTime();
                  const lastMicros = window.lastConvertedMicros || 0;
                  const lastTimeInMicros = lastTime * 1000 + lastMicros;
                  console.log(`  上一个时间: ${window.lastConvertedTimestamp}`);
                  console.log(`  lastTimeInMicros: ${lastTimeInMicros}`);
                  console.log(`  时间差: ${(currentDate.getTime() * 1000 + currentMicros) - lastTimeInMicros} 微秒`);
                }
              }
              ...
              if (window.lastConvertedTime && window.lastConvertedDate) {
                ...
                const currentTimeInMicros = currentTime * 1000 + currentMicros;
                const lastTimeInMicros = lastTime * 1000 + lastMicros;

                if (currentTimeInMicros < lastTimeInMicros) {
                  if (first === '540718') {
                    console.warn(`  时间倒退检测!`);
                    console.warn(`    当前: ${localTimestamp}`);
                    console.warn(`    上一个: ${window.lastConvertedTimestamp}`);
                  }

                  // 设置为上一个时间 + 1微秒
                  const newTime = lastTime + 1;
                  const newDate = new Date(newTime);
                  const newMicros = 1; // 1微秒
                  ...
                  const nmin = String(newDate.getMinutes()).padStart(2, '0');
                  const ns = String(newDate.getSeconds()).padStart(2, '0');
                  const nms = String(newDate.getMilliseconds()).padStart(3, '0');
                  const nus = String(newMicros).padStart(3, '0');

                  // 更新 localTimestamp
                  const correctedTimestamp = `[${ny}-${nm}-${nd} ${nh}:${nmin}:${ns}.${nms}${nus}]`;
                  window.lastConvertedDate = newDate;
                  window.lastConvertedMicros = newMicros;
                  window.lastConvertedTimestamp = correctedTimestamp;

                  if (first === '540718') {
                    console.warn(`    修正后: ${correctedTimestamp}`);
                  }

                  // 返回修正后的时间戳
                  return `${correctedTimestamp},${first},${second},${nanosStr},${rest}`;
                }
              }

              window.lastConvertedDate = currentDate;
              window.lastConvertedMicros = currentMicros;
              window.lastConvertedTimestamp = localTimestamp;
            }

            // 返回转换后的行
            return `${localTimestamp},${first},${second},${nanosStr},${rest}`;
          }

          console.warn('没有可用的UTC参考点，window.utcReferencePoints =', window.utcReferencePoints);

          // 兜底逻辑：如果没有找到UTC参考点，使用文件名提取的基准时间
          // （这个逻辑不太准确，但至少能提供一些参考）
          if (utcBaseTimestamp !== null) {
            const timestampMs = utcBaseTimestamp + Math.floor(nanos / 1000000);
            ...
            // 处理微秒部分（与主逻辑相同的方式）
            const microsMillis = Math.floor(remainingMicros / 1000);
            const microsOnly = remainingMicros % 1000;
            let totalMillis = date.getMilliseconds() + microsMillis;
          ...
          console.warn('没有可用的基准时间，utcBaseTimestamp =', utcBaseTimestamp);
          // 如果没有任何基准时间，无法转换
          return null;
        }

        // 转换时间戳按钮事件 - 使用WASM加速转换
        if (logCtxConvertTimestamp) {
          logCtxConvertTimestamp.addEventListener("click", async () => {
            try {
              // 检查WASM是否可用
              if (typeof WasmTimestampConverter !== 'undefined') {
                // 使用WASM转换
                await convertUTCTimeWithWasm();
              } else {
                // 降级到JavaScript转换
                showMessage("WASM不可用，使用JavaScript模式转换UTC时间...");
                await convertUTCTimeWithJavaScript();
              }
            } catch (error) {
              console.error('UTC转换错误:', error);
              showMessage(`UTC转换出错: ${error.message}`);
            }
          });
        }

        /**
         * JavaScript模式UTC转换（降级方案）
         */
        async function convertUTCTimeWithJavaScript() {
          console.log('🚀 [JavaScript模式] 开始UTC时间转换...');

          // 1. 检查UTC参考点（如果文件加载时已提取，应该已经有了）
          if (!window.utcReferencePoints || window.utcReferencePoints.length === 0) {
            console.log('UTC参考点为空，正在提取...');
            extractUTCReferencePoints(originalLines);
          }
          ...
          console.log(`✓ 使用 ${window.utcReferencePoints.length} 个UTC参考点进行转换`);
          console.log('参考点列表:', window.utcReferencePoints.map((ref, i) =>
            `[${i}] bootTime=${ref.bootTime}, time=${ref.utcTime.toISOString().substring(0, 19)}`
          ));

          // 【单调性保证】初始化单调性检查变量
          window.lastConvertedTime = null;
          window.lastConvertedDate = null;
          window.lastConvertedMicros = null;
          ...
          console.log(`开始转换 ${totalLines} 行日志...`);
          ...
          for (let i = 0; i < originalLines.length; i++) {
            ...
            // 使用JavaScript函数转换
            const convertedLine = convertTimestampToUTC(line);
            if (convertedLine) {
              originalLines[i] = convertedLine;
              convertedCount++;
            } else {
              skippedCount++;
            }
            ...
            // 每转换 10000 行输出一次进度
            if ((i + 1) % 10000 === 0) {
              console.log(`转换进度: ${i + 1}/${totalLines} (${((i + 1) / totalLines * 100).toFixed(1)}%)`);
            }
          }

          console.log(`✅ JavaScript转换完成: ${convertedCount} 行转换成功, ${skippedCount} 行跳过`);
          ...
          showMessage(`✓ 已使用JavaScript模式转换 ${convertedCount} 行时间戳`);
          hideLogContextMenu();
        }
      ...
      function initFilterContextMenu() {
        ...
        const filterPanel = document.getElementById('filteredPanel');
        const filterPanelContent = document.getElementById('filteredPanelContent');
        ...
        if (convertFilteredToUTCBtn) {
          convertFilteredToUTCBtn.addEventListener('click', async () => {
            try {
              showMessage("正在使用JavaScript模式转换UTC时间...");

              // 获取当前过滤结果的原始索引
              const indicesToConvert = currentFilter.filteredToOriginalIndex || [];

              // 1. 检查UTC参考点（文件加载时应该已经提取了）
              if (!window.utcReferencePoints || window.utcReferencePoints.length === 0) {
                console.log('UTC参考点为空，正在提取...');
                extractUTCReferencePoints(originalLines);
              }
              ...
              let convertedCount = 0;
              let skippedCount = 0;
              const totalLines = indicesToConvert.length;

              console.log(`开始转换过滤结果 ${totalLines} 行...`);
              ...
              for (let i = 0; i < indicesToConvert.length; i++) {
                ...
                if (originalIndex >= 0 && originalIndex < originalLines.length) {
                  ...
                  const convertedLine = convertTimestampToUTC(line);
                  if (convertedLine) {
                    originalLines[originalIndex] = convertedLine;
                    convertedCount++;
                  } else {
                    skippedCount++;
                  }
              ...
              console.log(`✅ 过滤结果转换完成: ${convertedCount} 行转换成功, ${skippedCount} 行跳过`);
              ...
              // 5. 同时更新过滤面板
              if (filteredPanelAllLines && filteredPanelAllLines.length > 0) {
                let filteredConvertedCount = 0;
                for (let i = 0; i < filteredPanelAllLines.length; i++) {
                  const convertedLine = convertTimestampToUTC(filteredPanelAllLines[i]);
                  if (convertedLine) {
                    filteredPanelAllLines[i] = convertedLine;
                    filteredConvertedCount++;
                  }
                }
                console.log(`过滤面板也更新了 ${filteredConvertedCount} 行`);

                // 更新过滤面板显示
                const filteredContent = filteredPanelContent;
                if (filteredContent) {
                  filteredContent.scrollTop = filteredContent.scrollTop;
                }
              }
          ...
          console.error('无法找到 convertFilteredToUTC 元素');
        }
      ...
      function updateLayout() {
        ...
        const width = isVisible ? fileTreeContainer.getBoundingClientRect().width : 0;
      ...
      function computeVisibleByExpandState() {
        ...
        for (let i = 0; i < fileTreeHierarchy.length; i++) {
          ...
          if (it.isArchive) {
            ...
            const isExpanded = it.isRemote ? it.expanded : expandedArchives.has(it.archiveName);
            if (!isExpanded) {
              collapsedLevels.push(lv);
            }
          }
        ...
        try {
          ...
          if (window.electronAPI && window.electronAPI.listArchive) {
            ...
            if (result.success) {
              ...
              // 7z 失败，检查是否是 ZIP 文件可以回退到 JSZip
              const errorMsg = result.error || '';
              if (archive.path.toLowerCase().endsWith('.zip')) {
                useJsjipFallback = true;
                console.log(`[loadLocalArchiveChildren] 7z 失败，ZIP 文件回退到 JSZip`);
              } else {
            ...
            for (const node of fileTreeHierarchy) {
              if (node && node._zipObject) {
                ...
                if (normalizedNodePath === normalizedParentPath ||
                    ...
                    node.name === 'WTGLMK-2064025.zip') {  // 特殊匹配文件名
      ...
      function buildFirstLevelNodes(files, parentArchive) {
        ...
        for (const [name, info] of firstLevelItems) {
          ...
          const node = {
            ...
            isLocalDrive: true,
            isNestedArchive: isNestedArchive,  // 🚀 标记为嵌套压缩包
            isArchive: true, // 🔧 标记为压缩包内容，用于 findParentArchive 查找父压缩包
            archiveName: parentArchive.path,
            archivePath: name,
            // 保存完整路径用于懒加载子项
...
3. 或者：拖拽整个压缩包（.zip/.rar等）
   - 支持直接浏览压缩包内容`);
      ...
      async function loadSelectedFiles() {
        ...
        const estimatedLinesPerFile = 100000;  // 假设每个文件10万行
        const estimatedTotalLines = originalLines.length + (selectedFiles.length * estimatedLinesPerFile);

        // 检查是否超过行数限制
        if (estimatedTotalLines > MAX_LINES) {
          const confirmed = confirm(
            `⚠️ 警告：即将加载大量数据\n\n` +
            `当前已加载: ${originalLines.length.toLocaleString()} 行\n` +
            `预计新增: ${(selectedFiles.length * estimatedLinesPerFile).toLocaleString()} 行\n` +
            `总计: ${estimatedTotalLines.toLocaleString()} 行（超过推荐值 ${MAX_LINES.toLocaleString()} 行）\n\n` +
            `建议：\n` +
            `1. 分批加载（每次选择较少文件）\n` +
            `2. 加载后使用过滤功能\n` +
            `3. 刷新页面后再加载\n\n` +
          ...
          if (!confirmed) {
            console.log('[loadSelectedFiles] 用户取消加载（行数限制）');
            showMessage('加载已取消');
            return;
          }
        }
        ...
        if (performance.memory) {
          ...
          console.log(`[内存监控] 当前使用: ${usedMB.toFixed(1)}MB / ${totalMB.toFixed(1)}MB`);
          ...
          if (usedMB > MAX_MEMORY_MB * 0.8) {  // 80%警告
            ...
            if (!shouldContinue) {
              console.log('[loadSelectedFiles] 用户取消加载（内存限制）');
              showMessage('加载已取消，建议刷新页面');
              return;
            }
          }
        }
        ...
        console.log(`[loadSelectedFiles] 已加载文件数: ${loadedFileIndices.size}, 新文件数: ${newFiles.length}, 增量模式: ${hasNewFiles}`);

        // 🔧 修复重复加载：如果没有任何新文件需要加载，直接返回
        if (!hasNewFiles) {
        ...
        if (hasNewFiles) {
          ...
          console.log(`[loadSelectedFiles] 增量加载模式：保留已有内容，只加载 ${newFiles.length} 个新文件`);
        } else {
          // 没有新文件，不重新加载
          console.log(`[loadSelectedFiles] 所有文件已加载，跳过`);
          return;
        }
    ...
    function clearAllFiles() {
        ...
        setTimeout(() => {
          ...
          console.log('unescapeCache:', unescapeCache.size);
      ...
      function getTargetContainer() {
        const filteredPanelEl = document.getElementById('filteredPanel');
        ...
        if (filteredPanelVisible) {
          return document.getElementById('filteredPanelContent');
        } else {
          ...
          for (const type of dt.types) {
            try {
              ...
              if (data && typeof data === 'string' && data.length > 0 && data.length < 10000) {
                // 🔧 优先检查是否为 Everything 搜索的拖拽数据
                if (type === 'application/json') {
                  try {
                    const dragData = JSON.parse(data);
                    if (dragData && dragData.source === 'everything-search') {
                      console.log("✅ 识别到 Everything 搜索拖拽数据:", dragData);

                      // 调用 Everything 的拖放处理
                      if (window.App && window.App.EverythingSearchUI && window.App.EverythingSearchUI.handleDropOnFileTree) {
                        await window.App.EverythingSearchUI.handleDropOnFileTree(dragData);
                        return;
                      }
                    }
                  } catch (parseError) {
...
2. 或直接从压缩软件拖拽文件到本应用`);
      ...
      async function readFileFromArchive(archiveName, filePath) {
        ...
        if (!archive) {
          ...
          const archiveIndex = fileTreeHierarchy.findIndex(item => {
            // 多种匹配方式
            return item.archiveName === archiveName ||
                   item.path === archiveName ||
                   (item.name === archiveName && item._archiveFiles);
          });
          ...
          if (archiveIndex !== -1) {
            ...
            // 🔧 如果有 _archiveFiles（7z 列出的文件列表），需要通过主进程读取
              ...
              const result = await window.electronAPI.extractFileFromArchive(archivePath, filePath);

              if (!result || !result.success) {
                throw new Error(result?.error || `读取压缩包文件失败: ${filePath}`);
              }
              ...
              if (result.symlinkWarning && result.symlinkMessage) {
                ...
                // 可以选择显示一个 toast 通知用户
                // showToast(result.symlinkMessage, 'warning');
              }
      ...
      async function readFileAsText(file) {
        ...
        // 如果所有编码都失败，尝试二进制读取 + 手动解码
        console.warn('⚠️ 所有文本编码失败，尝试二进制读取');
        try {
          ...
          // 尝试用 TextDecoder 自动检测
          const decoder = new TextDecoder('utf-8', { fatal: false });
          const result = decoder.decode(arrayBuffer);
          console.log('📖 文件编码: 自动检测');
          return result;
        } catch (error) {
          throw new Error(`无法读取文件 ${file.name}，可能是二进制文件或编码不支持`);
        }
      }
      ...
      async function loadFileStreaming(file, fileName, chunkSize, onProgress, sessionId, filePath = null) {
        ...
        // 提取UTC参考点（在文件加载完成后）
        console.log(`[loadFileStreaming] 文件 ${fileName} 加载完成，准备提取UTC参考点...`);
        const fileLines = originalLines.slice(startIndex + 1, startIndex + 1 + lineCount);
        console.log(`[loadFileStreaming] 文件有 ${fileLines.length} 行`);
        extractUTCReferencePoints(fileLines);
        console.log(`[loadFileStreaming] UTC参考点提取完成`);
      ...
      function processFileContent(content, fileName, filePath = null, fileTreeIndex = null) {
        ...
        // 尝试从文件名中提取基准时间（用于 UTC 时间转换）
        // 文件名格式：kernel_log_3__2025_1228_102627 或类似格式
        extractBaseTimestampFromFileName(fileName);

        // 修改：保持文件内容原封不动，不进行转义
        // 只对文件名进行转义以确保安全
        const lines = content.split("\n");

        console.log(`[processFileContent] 分割后得到 ${lines.length} 行`);
        ...
        if (lines.length > 0) {
          ...
          // 🔧 启用 UTC 参考点提取（确保时间转换连续性）
          console.log(`[processFileContent] 准备提取UTC参考点...`);
          extractUTCReferencePoints(lines);
          console.log(`[processFileContent] UTC参考点提取完成，当前全局参考点数: ${window.utcReferencePoints?.length || 0}`);
      ...
      function extractBaseTimestampFromFileName(fileName) {
        ...
        // 例如：kernel_log_3__2025_1228_102627
        // 🔧 注意：文件名中的时间戳通常是**本地时间**（文件创建时间）
        ...
        if (match) {
          ...
          // 创建本地时间日期对象（文件名时间是本地时间）
          const localDate = new Date(year, month - 1, day, hour, minute, second);
          utcBaseTimestamp = localDate.getTime();

          console.log(`从文件名提取基准时间(本地): ${fileName} -> ${localDate.toString()}`);
          console.log(`  ISO格式: ${localDate.toISOString()}`);
          console.log(`  UTC时间: ${localDate.toUTCString()}`);
        }
      }

      // 从日志内容中提取UTC参考点（参考klog脚本的逻辑）
      function extractUTCReferencePoints(lines) {
        console.log(`[extractUTCReferencePoints] 开始处理，共 ${lines.length} 行`);

        // 初始化全局变量（如果还没有）
        if (!window.utcReferencePoints) {
          window.utcReferencePoints = [];
        }

        const newReferencePoints = [];

        // 【关键改进】将文件名基准时间作为第一个参考点（boot_time=0）
        // 这样可以确保所有时间都有起点，并且时间单调递增
        if (utcBaseTimestamp !== null && !window.utcReferencePoints.some(p => p.bootTime === 0)) {
          const baseTime = new Date(utcBaseTimestamp);
          newReferencePoints.push({
            lineIndex: -1, // -1 表示来自文件名
            bootTime: 0,   // boot_time=0
            utcTime: baseTime,
            isFileNameBase: true // 标记为文件名基准
          });
          console.log(`[extractUTCReferencePoints] ✓ 添加文件名基准时间: boot=0, 时间=${baseTime.toISOString().substring(0, 19)}`);
        }

        // 匹配两种模式的UTC参考点：
        // 1. "YYYY-MM-DD HH:MM:SS.mmmmmm UTC;android time YYYY-MM-DD HH:MM:SS.mmmmmm"
        // 2. "android time YYYY-MM-DD HH:MM:SS.mmmmmm"
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (line.includes('android time')) {
            androidTimeCount++;
            console.debug(`[extractUTCReferencePoints] 找到android time行 ${i}: ${line.substring(0, 80)}...`);
          }

          // 提取boot time（日志行中的第三个字段）
          const bootTime = extractBootTime(line);
          if (!bootTime) {
            if (line.includes('android time')) {
              console.warn(`[extractUTCReferencePoints] android time行无法提取boot time: ${line.substring(0, 80)}`);
            }
            continue;
          }
          ...
          if (match) {
            console.debug(`[extractUTCReferencePoints] 匹配到带UTC标记的android time`);
            matchedCount++;
            // 解析Android time
            const androidTimeStr = match[2].replace(' ', 'T');

            // 🔧 修复：始终将 android time 作为 UTC 时间
            // Android time 是系统的UTC时间戳，需要加 'Z' 后缀
            referenceDateTime = new Date(androidTimeStr + 'Z');
            console.debug(`[extractUTCReferencePoints] android time 作为 UTC 时间: ${androidTimeStr}Z`);
          } else {
            // 匹配不带UTC标记的行
            match = line.match(timePattern);
            if (match) {
              console.debug(`[extractUTCReferencePoints] 匹配到不带UTC标记的android time`);
              matchedCount++;
              // 🔧 修复：不加'Z'，让Date对象将其解析为本地时间（对于不包含UTC标记的情况）
              // 但根据实际日志，android time 应该始终是 UTC 时间
              // 这里也改为始终作为 UTC 时间
              referenceDateTime = new Date(match[1].replace(' ', 'T') + 'Z');
              console.debug(`[extractUTCReferencePoints] android time 作为 UTC 时间: ${match[1]}Z`);
            }
          }
          ...
          if (referenceDateTime && !isNaN(referenceDateTime.getTime())) {
            newReferencePoints.push({
              lineIndex: i,
              bootTime: bootTime,
              utcTime: referenceDateTime
            });
            console.log(`[extractUTCReferencePoints] ✓ 添加UTC参考点: 行${i}, boot=${bootTime}, 时间=${referenceDateTime.toISOString().substring(0, 19)}`);
          } else if (line.includes('android time')) {
            console.warn(`[extractUTCReferencePoints] android time行解析失败: ${line.substring(0, 80)}`);
          }
        }

        console.log(`[extractUTCReferencePoints] 共找到 ${androidTimeCount} 个android time行，匹配成功 ${matchedCount} 个`);

        // 按boot_time排序
        newReferencePoints.sort((a, b) => a.bootTime - b.bootTime);

        if (newReferencePoints.length > 0) {
          console.log(`✓ 提取到 ${newReferencePoints.length} 个UTC参考点`);
          newReferencePoints.forEach((ref, idx) => {
            const timeStr = ref.utcTime.toISOString().replace('T', ' ').substring(0, 19);
            console.log(`  [${idx + 1}] 行${ref.lineIndex}, boot=${ref.bootTime} 微秒, 时间=${timeStr}`);
          });

          // 合并到全局列表（避免重复）
          const existingBootTimes = new Set(window.utcReferencePoints.map(p => p.bootTime));
          for (const ref of newReferencePoints) {
            if (!existingBootTimes.has(ref.bootTime)) {
              window.utcReferencePoints.push(ref);
              existingBootTimes.add(ref.bootTime);
            }
          }
          // 重新排序全局列表
          window.utcReferencePoints.sort((a, b) => a.bootTime - b.bootTime);
          console.log(`[extractUTCReferencePoints] 全局UTC参考点总数: ${window.utcReferencePoints.length}`);
        } else {
          console.warn('⚠ 未找到UTC参考点，将使用文件名时间作为基准');
        }

        return window.utcReferencePoints;
      }
      ...
      function extractBootTime(line) {
        ...
        // 匹配kernel日志格式: "[<6>] [    0.123456] ..."
        const kmsgMatch = line.match(/(<\d+>)?\[\s*(\d+)\.(\d+)\]/);
        if (kmsgMatch) {
          const seconds = parseInt(kmsgMatch[2], 10);
          const microseconds = parseInt(kmsgMatch[3], 10);
      ...
      function startIncrementalFilter(keywords, compiledPatterns, getCachedUnescape) {
        ...
        function finishFiltering() {
          ...
          console.log('- Cache size:', unescapeCache.size);
          // 🚀 优化：避免输出大数组
          ...
          if (lastClickedOriginalIndex >= 0) {
            ...
            const targetFilteredIndex = originalToFilteredMap.get(lastClickedOriginalIndex);

            if (targetFilteredIndex !== undefined) {
              console.log(`[Filter MainThread] ✅ 找到匹配行，耗时: O(1), 索引: ${targetFilteredIndex}`);
              lastClickedFilteredIndex = targetFilteredIndex;

              // 等待 DOM 更新后执行跳转
              setTimeout(() => {
                const fpc = document.getElementById('filteredPanelContent');
                const fpvc = document.getElementById('filteredPanelVirtualContent');
              ...
              console.log(`[Filter MainThread] ℹ️ 该行不在新过滤结果中，已清除记录`);
      ...
      function cleanLogData() {
        ...
        // 7. 清空书签数据
        bookmarks = [];
        bookmarkedIndexSet.clear();
        window.bookmarksByFile = {};
      ...
      function updateVisibleLines() {
        ...
        const newVisibleStart = Math.max(
          0,
          Math.floor(scrollTop / lineHeight) - bufferSize
        );
        const newVisibleEnd = Math.min(
          originalLines.length - 1,
          Math.ceil((scrollTop + clientHeight) / lineHeight) + bufferSize
        );
        ...
        for (let i = visibleStart; i <= visibleEnd; i++) {
          ...
          // 书签标记
          if (!isFileHeader && bookmarkedIndexSet && bookmarkedIndexSet.has(i)) {
          ...
          // 🚀 性能优化：使用transform替代top，启用GPU加速
          line.style.transform = `translateY(${Math.floor(i * lineHeight)}px)`;
          ...
          if (cachedHtml !== null) {
            ...
            // 应用搜索高亮
            if (hasSearchKeyword && !isFileHeader) {
              displayContent = safeHighlight(
                displayContent,
                searchKeyword,
                (match) => {
                  const isCurrent =
                    i === currentMatchLine &&
                    match.toLowerCase() === searchKeyword.toLowerCase();
                  return isCurrent
                    ? `<span class="current-search-highlight">${match}</span>`
                    : `<span class="search-highlight">${match}</span>`;
                }
            ...
            // 应用自定义高亮
            if (hasCustomHighlights && !isFileHeader) {
              for (let h = 0; h < customHighlights.length; h++) {
                const highlight = customHighlights[h];
                if (!highlight.keyword) continue;
                displayContent = safeHighlight(
                  ...
                  highlight.keyword,
                  (match) =>
                    `<span class="custom-highlight" style="background-color: ${highlight.color}80;">${match}</span>`
                );
              }
            }

            // 🔧 修复：主日志框不应用过滤关键词高亮（过滤关键词高亮只在过滤面板中显示）

            // 已移除HTML链接转换功能 - 日志工具不需要将 .html 路径转换为链接
            // displayContent = convertHtmlLinks(displayContent);

            // 🚀 性能优化：将解析结果存入缓存
            setCachedHtml(lineContent, cacheKey, displayContent);
          }

          // 🚀 性能优化：行号已改用CSS计数器实现，无需创建HTML元素
          // 直接设置内容，行号由CSS ::before 伪元素自动生成
...
async function applyFilterWithRipgrepAsync(filterText) {
  try {
    ...
    // 首先按 | 分割多个 OR 组
    const orGroups = filterText.split('|').map(k => k.trim()).filter(k => k);

    const patterns = [];
    for (const group of orGroups) {
      // 按空格分割 AND 词
      const words = group.split(/\s+/).filter(w => w.trim());
      ...
      if (words.length === 1) {
        ...
        // 多个词：生成所有排列组合
        // 例如：["battery", "l"] → "battery[^a-zA-Z0-9_]*l|l[^a-zA-Z0-9_]*battery"
        const permutations = getPermutations(words);
        const wordPatterns = permutations.map(p => {
          // 每个词之间用 [^a-zA-Z0-9_]* 连接，表示可以有非字母数字下划线字符
          return p.join('[^a-zA-Z0-9_]*');
        });
        patterns.push(`(${wordPatterns.join('|')})`);
      }
    }

    const rgPattern = patterns.join('|');
    ...
    for (const match of matches) {
      const originalIndex = findOriginalIndexForFileInRipgrep(match.filePath, match.lineNumber);
...
async function applyFilterWithRipgrepOnFiles(filterText, filePaths) {
  try {
    ...
    // 如果包含压缩包文件，说明筛选逻辑有问题，记录警告
    const hasArchiveFiles = filePaths.some(path => path.match(/\.(zip|tar|gz|7z|rar)\//i));
    if (hasArchiveFiles) {
      console.warn(`[过滤模式] 警告：文件列表中包含压缩包文件，这些文件将被跳过`);
    }
    ...
    // - 单个字母视为独立标记（前后要有分隔符）
    // 例如：
    // - "battery l" → "battery" 后面跟着独立标记 "l"（如 "l=100"）
    // - "battery|adf" → 包含 "battery" 或 "adf"
    ...
    for (const group of orGroups) {
      ...
      if (words.length === 1) {
        ...
        // 多个词：生成所有排列组合
        // 例如：["battery", "l"] → "battery[^a-zA-Z0-9_]*[^a-zA-Z0-9_]*l|l[^a-zA-Z0-9_]*[^a-zA-Z0-9_]*battery"
        // 其中 [^a-zA-Z0-9_]* 匹配分隔符（空格、标点、换行等）
