...
async function applyFilterWithRipgrep() {
  ...
  if (targetFilteredIndex >= 0) {
    console.log(`[Ripgrep Filter] 跳转到之前的选中行: ${targetFilteredIndex}`);
...
function isFromArchive(files) {
  ...
  if (!firstFile || !firstFile.path) return true;  // 没有路径视为压缩包

  const path = firstFile.path;

  // 检查路径格式
  // 1. 包含压缩包标记（如 "archive.zip:file.log" 或 "archive.7z:file.log"）
  if (path.includes('.zip:') || path.includes('.7z:') || path.includes('.tar:') || path.includes('.gz:')) {
    return true;
  }

  // 2. 检查是否有 archiveName 属性（压缩包特有的属性）
  if (firstFile.archiveName || firstFile.fromArchive) {
    return true;
  }

  // 3. 检查路径是否存在（如果是压缩包内的文件，路径可能不存在）
  // 注意：这里不能同步检查文件是否存在，会阻塞
  // 所以只检查路径格式

  // 4. 检查是否是有效路径（Windows路径通常包含盘符）
  // 压缩包内的文件通常没有盘符，或格式特殊
  if (!path.match(/^[A-Za-z]:\\/)) {  // 不是标准的 Windows 绝对路径
    // 可能是压缩包内的文件，但不一定
    // 需要结合其他特征判断
  }
  ...
  if (/^[A-Za-z]:\\/.test(path)) {
    ...
    for (const pattern of archivePatterns) {
      if (pattern.test(path)) {
        console.log(`[Filter] 路径包含压缩包标记: ${path}`);
        return false;
      }
    }
...
async function applyFilterWithRipgrepAsync(filterText, headers) {
  try {
    ...
    console.log('[Ripgrep Filter] 🔍 调试: lastClickedFilteredIndex =', typeof lastClickedFilteredIndex !== 'undefined' ? lastClickedFilteredIndex : 'undefined');
    console.log('[Ripgrep Filter] 🔍 调试: filteredPanelAllOriginalIndices.length =', typeof filteredPanelAllOriginalIndices !== 'undefined' ? filteredPanelAllOriginalIndices.length : 'undefined');

    if (typeof lastClickedFilteredIndex !== 'undefined' && lastClickedFilteredIndex >= 0 && filteredPanelAllOriginalIndices && filteredPanelAllOriginalIndices.length > 0) {
      if (lastClickedFilteredIndex < filteredPanelAllOriginalIndices.length) {
        rememberedOriginalIndex = filteredPanelAllOriginalIndices[lastClickedFilteredIndex];
        console.log('[Ripgrep Filter] ✅ 记录当前选中行: filteredIndex=' + lastClickedFilteredIndex + ' -> originalIndex=' + rememberedOriginalIndex);
      } else {
        console.warn('[Ripgrep Filter] ⚠️ lastClickedFilteredIndex 超出范围: ' + lastClickedFilteredIndex + ' >= ' + filteredPanelAllOriginalIndices.length);
      }
    } else {
      console.log('[Ripgrep Filter] ℹ️ 当前没有选中行，无法记忆位置');
    }
...
function applyFilterWithParallelWorkers(keywords) {
  ...
  // 🚀 智能跳转：在过滤开始前就记录当前选中行的原始索引
  let rememberedOriginalIndex = -1;
  if (typeof lastClickedFilteredIndex !== 'undefined' && lastClickedFilteredIndex >= 0 && filteredPanelAllOriginalIndices) {
    rememberedOriginalIndex = filteredPanelAllOriginalIndices[lastClickedFilteredIndex];
  }
  ...
  function onRenderComplete(totalTime, stats, useSharedWorker, rememberedOriginalIndex) {
    ...
    setTimeout(() => {
      ...
      // 🚀 如果找到了匹配的行，跳转并高亮；否则重置
      if (targetFilteredIndex >= 0) {
        console.log(`[ParallelFilter] 跳转到之前的选中行: ${targetFilteredIndex}`);
        lastClickedFilteredIndex = targetFilteredIndex;
...
function applyFilterWithSingleWorker(keywords) {
  ...
  if (typeof lastClickedFilteredIndex !== 'undefined' && lastClickedFilteredIndex >= 0 && filteredPanelAllOriginalIndices) {
    rememberedOriginalIndex = filteredPanelAllOriginalIndices[lastClickedFilteredIndex];
    console.log('[Filter SingleWorker] 记录当前选中行:', rememberedOriginalIndex);
  }
...
function applyWorkerResults(filteredLines, filteredToOriginalIndex, stats, rememberedOriginalIndex = -1) {
  ...
  // 🚀 如果找到了匹配的行，跳转并高亮；否则重置
  if (targetFilteredIndex >= 0) {
    console.log(`[Filter Worker] 跳转到之前的选中行: ${targetFilteredIndex}`);
    lastClickedFilteredIndex = targetFilteredIndex;
