...
const highlightCache = {
  ...
  cache: new Map(),
  maxSize: 1000,  // 🚀 优化：减少缓存大小（10000 → 1000），内存减少45MB
...
function renderLogLines() {
  ...
  // 使用 transform 代替 height，解决大文件性能问题
  const totalHeight = originalLines.length * lineHeight;
  if (totalHeight > 10000000) {
    // 对于超大文件，使用 transform scaleY
    placeholder.style.cssText = 'position:absolute;width:1px;height:1px;transform-origin:top;transform:scaleY(' + totalHeight + ');';
  } else {
    // 对于正常大小，直接设置高度
...
function updateVisibleLines() {
  ...
  highlightCache.updateState(searchKeyword, customHighlights, currentFilter.filterKeywords || []);

  // 渲染可见行
  const hasSearchKeyword = !!searchKeyword;
  const hasCustomHighlights = customHighlights.length > 0;
  const hasFilterKeywords = currentFilter.filterKeywords && currentFilter.filterKeywords.length > 0;
  ...
  for (let i = visibleStart; i <= visibleEnd; i++) {
    ...
    line.style.top = Math.floor(i * lineHeight) + "px";
    line.dataset.lineNumber = i + 1;

    // 🚀 性能优化：检查是否需要高亮
    const needsHighlight = hasSearchKeyword || hasCustomHighlights || hasFilterKeywords;
    ...
    if (!needsHighlight && !isFileHeader) {
      ...
      const displayContent = isFileHeader
        ...
        : highlightContent(lineContent, {
            ...
            filterKeywords: hasFilterKeywords ? currentFilter.filterKeywords : [],
            lineIndex: i,
...
function jumpToOriginalLine(originalIndex) {
  ...
  // 计算目标行的位置
  const targetLineTop = originalIndex * lineHeight;
