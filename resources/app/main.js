...
ipcMain.handle('call-es', async (event, options) => {
  const { execPath = 'C:\\Program Files\\Everything\\es.exe', args = [] } = options;

  try {
    console.log('[call-es] 调用 es.exe, 参数:', args);

    // 检查 es.exe 是否存在
    if (!fs.existsSync(execPath)) {
      return {
        success: false,
        error: `es.exe 不存在: ${execPath}\n请安装 Everything 或修改 es.exe 路径`
      };
    }
  ...
  try {
    ...
    if (platform === 'win32') {
      ...
      for (const letter of possibleDrives) {
        ...
        console.log(`[get-data-drives] 检查驱动器: ${drivePath}`);

        try {
          // 检查驱动器是否存在并可访问
          const exists = fs.existsSync(drivePath);
          console.log(`[get-data-drives]   fs.existsSync(${drivePath}): ${exists}`);

          if (exists) {
            // 尝试读取目录确认可访问
            const entries = fs.readdirSync(drivePath);
            console.log(`[get-data-drives]   ${drivePath} 可访问，包含 ${entries.length} 个项`);
          ...
          // 驱动器不存在或不可访问，跳过
          console.log(`[get-data-drives]   ${drivePath} 跳过: ${e.message}`);
        }
      }
    } else {
    ...
    console.log(`[get-data-drives] 完成，发现 ${drives.length} 个数据盘:`, drives);
